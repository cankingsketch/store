...
/* 「空罐王商品後台」這個應用程式的 Application Audience (AUD) tag。
 * 沒有這個檢查的話，同一個 Cloudflare 團隊底下「任何」應用程式簽出來的
 * token 都能通過——例如規劃器 planner.cankingstore.com 的。那不是金鑰，
 * Cloudflare 自己會把它放在公開的登入跳轉網址裡。 */
const AUD = '2bdfa72748cd73e6b2ef2ecdf4eefe834d0ba15fe941569878721b91230b299e';
...
async function verify(token) {
  ...
  const audOk = aud === AUD || (Array.isArray(aud) && aud.indexOf(AUD) >= 0);
  if (!audOk) return { reason: 'wrong-audience' };
